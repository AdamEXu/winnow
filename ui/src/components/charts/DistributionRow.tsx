...
export default function DistributionRow({ def, corpus, value }: DistributionRowProps) {
  let min = Infinity;
  let max = -Infinity;
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-40 shrink-0 truncate text-xs text-ink2" title={def.gloss}>
        {def.label}
      </div>
      <div className="min-w-0 flex-1">
        <svg viewBox={`0 0 ${W} ${H}`} className="block h-6 w-full" preserveAspectRatio="none" aria-hidden="true">
          ...
          <line x1={xOf(value)} y1={2} x2={xOf(value)} y2={H - 2} stroke="var(--color-blue)" strokeWidth={2} vectorEffect="non-scaling-stroke" />
          <circle cx={xOf(value)} cy={H / 2} r={3.5} fill="var(--color-blue)" stroke="var(--color-mount)" strokeWidth={1.5} />
        </svg>
      </div>
